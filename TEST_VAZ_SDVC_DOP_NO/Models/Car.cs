...
namespace TEST_VAZ_SDVC.Models
{
	internal class Car : DependencyObject, INotifyPropertyChanged
	{
		...
		#region prisets
		/// Присет - верхняя граница при достижении которой звук увеличится на 1 дБ.
		static private int acceleratePrisetIndex = 0;
		/// Присет - нижняя граница при достижении которой звук уменьшиться на 1 дБ.
		...
		private void ChangeVolumeSDVC(int speed, bool aceleration)
		{
			if (aceleration) 
			{
				if (speed > accelerate[acceleratePrisetIndex]) 
				{
					...
					deceleratePrisetIndex = acceleratePrisetIndex;
					volumeSDVC++;
				}
			}
			else
			{
				if(speed < decelerate[deceleratePrisetIndex]) 
				{
					...
					acceleratePrisetIndex = deceleratePrisetIndex;
					volumeSDVC--;
				}
			}
		}
		...
		public string volumeSum 
		{
			get 
			{
				...
				if (volumeGain == null)
				{
					_volumeSDVC = deceleratePrisetIndex;
					return "MUTE";
				}
