...
namespace TEST_VAZ_SDVC.Models
{
	internal class Car : DependencyObject, INotifyPropertyChanged
	{
		...
		private void ChangeVolumeSDVC(int speed, bool dictionary)
		{
			int gainValue;
			if (dictionary) 
			{
			...
			else
			{
				if(speed < decelerate[deceleratePrisetIndex]) 
				{
					...
					volumeSDVC = acceleratePrisetIndex = deceleratePrisetIndex = Math.Min(volumeSDVC, deceleratePrisetIndex);
				}
			}
		}
		#endregion
